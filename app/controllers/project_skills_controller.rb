class ProjectSkillsController < ApplicationController
  before_action :set_project_skill, only: %i[ show update destroy ]
  ...
  def create
    ...
    if @project_skill.save
      render json: @project_skill, serializer: ProjectSkillSerializer,status: :created, location: @project_skill
    else
  ...
  private
    ...
    def project_skill_params
      params.require(:project_skill).permit(:skill_id, :project_id, :position, :description, :user_id)
    end
end
